...
export default function WaysToEarnContainer({ category }: WaysToEarnContainerProps) {
  ...
  const applyWaysToEarnFilter = (websitesToFilter: Website[], targetCategory: string) => {
  ...
  // Apply additional filters (from FilterSidebar) on top of category filtering
  const applyFilters = (websitesToFilter: Website[], filterCriteria: FilterCriteria) => {
    ...
    // Apply additional ways to earn filter (on top of category filter)
        ...
        {filteredWebsites.length === 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
            ...
            <p className="text-gray-600">
              {hasActiveFilters 
                ? `No websites match your current filters for ${category.toLowerCase()}.`
                : `We couldn't find any websites that offer ${category.toLowerCase()} opportunities.`
              }
