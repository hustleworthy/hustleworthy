...
interface PayoutMethodsFilterSidebarProps {
...
export default function PayoutMethodsFilterSidebar({
  filters,
  basePath,
  hasActiveFilters,
  isOpen,
  onToggle,
}: PayoutMethodsFilterSidebarProps) {
  ...
  const buildFilterUrl = (nextFilters: FilterCriteria) => {
  ...
  const updateFilters = (updates: Partial<FilterCriteria>) => {
    ...
    router.push(buildFilterUrl(nextFilters))
  }
    ...
    <>
      ...
      >
        ...
        <div className="p-4 lg:p-6 space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Expert Rating</label>
            <select
          ...
          <div ref={waysDropdownRef}>
            <label className="block text-sm font-medium text-gray-700 mb-2">Ways to Earn</label>
            <div className="relative">
              <button
                type="button"
              ...
              >
                ...
                <svg
                  className={`w-4 h-4 transition-transform ${isWaysDropdownOpen ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                    ...
                    {waysToEarnOptions.map((option) => (
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              updateFilters({ waysToEarn: [...filters.waysToEarn, option] })
                            } else {
                              updateFilters({
          ...
          <div>
            <label className="flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) => updateFilters({ investmentRequired: e.target.checked })}
