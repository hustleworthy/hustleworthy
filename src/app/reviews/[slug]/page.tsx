...
export default async function ReviewPage({ params }: ReviewPageProps) {
  ...
  const expertRatingValue = Number.parseFloat((website.expertRating || '0').toString().split(' ')[0]) || 0
  const ratingCount = website.reviews.filter((review) => typeof review.rating === 'number').length
  const userRatingSum = website.reviews.reduce((sum, review) => sum + (review.rating || 0), 0)
  const totalRatings = ratingCount + (expertRatingValue > 0 ? 1 : 0)
  const aggregateRatingValue =
    totalRatings > 0 ? (userRatingSum + expertRatingValue) / totalRatings : 0
  ...
  const reviewDates = website.reviews.map((review) => new Date(review.createdAt).getTime()).filter(Number.isFinite)
